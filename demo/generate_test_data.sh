...
export DB_IP=${INFLUXDB_PORT_2003_TCP_ADDR}
#export DB_IP=${CLOUDBANDWIDTH_CARBON_1_PORT_2003_TCP_ADDR}
export DB_PORT=${INFLUXDB_PORT_2003_TCP_PORT}
...
while [ $(( ( i += 1 ) <= $ENTRY_COUNT )) -ne 0 ]; do
    ...
    if [ "$i" -eq "${ENTRY_COUNT}" ]; then
        echo "Test data generation finished writing [${ENTRY_COUNT}] data points and is now complete. Next try some real metrics!  "
        exit 0
    fi
done
